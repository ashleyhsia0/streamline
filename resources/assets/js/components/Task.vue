<template>
    <li>
        <div class="checkbox">
            <label>
                <input type="checkbox" value="" :id="task.id" :value="task.id" @click="toggleCheck(task.id, index)" :checked="task.status === 1 || task.status === 2">
                ID#{{ task.id }}
                {{ task.title }}
            </label>
        ...
        <ul v-show="open" v-if="isParent">
            <task v-for="(task, index) in task.descendants" :task="task" :key="task.id" :index="index" :toggleCheck="toggleCheck" :getTasksByStatus="getTasksByStatus" :status="status"></task>
        </ul>
    </li>
</template>
